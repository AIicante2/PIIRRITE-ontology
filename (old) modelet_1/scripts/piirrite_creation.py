...
def add_OsmConceptScheme_to_piirritev(piirritev_graph:Graph, osm_key:str, description:str) -> None:
    ConceptScheme = piirritev[f'{snake_to_camel(osm_key)}']
    piirritev_graph.add((ConceptScheme, RDF.type, SKOS.ConceptScheme))
    piirritev_graph.add((ConceptScheme,
                      SKOS.prefLabel,
                      Literal(f'{snake_to_natural(osm_key, True)}, types', lang = 'en')))
    piirritev_graph.add((ConceptScheme, SKOS.definition, Literal(description, lang = 'en')))
    piirritev_graph.add((ConceptScheme, SKOS.note, Literal('osmId:' + osm_key + ';;')))

def add_OsmConcept_to_piirritev(piirritev_graph:Graph, osm_key:str, value:str, description:str) -> None:
    if value == 'any':
        return
    Concept = piirritev[f'{snake_to_camel(osm_key.replace(' ', '_'))}{snake_to_camel(value)}']
    piirritev_graph.add((Concept, RDF.type, SKOS.Concept))
    piirritev_graph.add((Concept, RDFS.subClassOf, piirrite.OsmEntity))
    piirritev_graph.add((Concept,
                      SKOS.prefLabel,
                      Literal(snake_to_natural(value, True), lang = 'en')))
    piirritev_graph.add((Concept, SKOS.definition, Literal(description, lang = 'en')))
    piirritev_graph.add((Concept, SKOS.inScheme, piirritev[f'{snake_to_camel(osm_key)}']))
    piirritev_graph.add((Concept, SKOS.note, Literal('osmId:' + value + ';;')))
...
def add_hasOsmTuic_to_piirrite2(piirritev_graph:Graph, piirrite2_graph:Graph,
    ...
    if snake_to_camel(tuic) != value_camel:
        ...
        tuic_camel = 'Type'
    hasOsmTuic = piirrite[f'has{value_camel}{tuic_camel}']
    piirrite2_graph.add((hasOsmTuic, RDF.type, OWL.DatatypeProperty))
    piirrite2_graph.add((hasOsmTuic, RDFS.subPropertyOf, saref.hasValue))
    piirrite2_graph.add((hasOsmTuic,
                      RDFS.label,
                      Literal(snake_to_natural(tuic, True),
                              lang = 'en')))
    # piirrite2_graph.add((hasOsmTuic, RDFS.comment, Literal(description, lang = 'en')))
    piirrite2_graph.add((hasOsmTuic, RDFS.domain, saref.Property))
    piirrite2_graph.add((hasOsmTuic, RDFS.range, RDFS.Literal))
    piirrite2_graph.add((hasOsmTuic, SKOS.note, Literal('osmId:' + tuic + ';;')))
    piirrite2_graph.add((hasOsmTuic, piirrite.isTagRelatedTo, piirritev[f'{snake_to_camel(osm_key)}{value_camel}']))
    piirritev_graph.add((piirritev[f'{snake_to_camel(osm_key)}{value_camel}'], piirrite.hasRelatedTag, hasOsmTuic))

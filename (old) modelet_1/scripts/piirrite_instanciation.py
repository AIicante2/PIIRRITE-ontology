...
piirrited = Namespace('http://piirrite.univ-lyon1.fr/data#')
...
def init_piirrite_graph() -> Graph:
    ...
    piirrite_graph.parse(TBox_file, 'turtle')
    piirrite_graph.parse(TBox2_file, 'turtle')
...
def init_piirritev_graph() -> Graph:
    ...
    piirritev_graph.parse(GoT_file, 'turtle')
...
def init_piirrited_graph() -> Graph:
    ...
    piirrited_graph.bind('piirrited', piirrited)
...
def add_geometry_to_PointOfInterest(piirrited_graph:Graph,
                                 osmd_graph:Graph,
                                 PointOfInterest:URIRef,
                                 osm_geometry) -> None:
    ...
    if len(geometries_as_WKT) == 1:
        ...
        geometry = BNode()
        piirrited_graph.add((PointOfInterest, geo.hasGeometry, geometry))
...
def add_context_to_PointOfInterest(osm_node:URIRef,
                                piirrite_graph:Graph, piirritev_graph:Graph, piirrited_graph:Graph, osmd_graph:Graph,
                                PointOfInterest:URIRef, osm_key:str, osm_value:str, unfounds:dict[str, dict[str, int]]) -> dict[str, dict[str, int]]:
    ...
    if len(osm_value.split(' ')) > 1 or not should_be_concept([osm_value]):
        ...
        if (piirritev[conceptScheme], RDF.type, SKOS.ConceptScheme) in piirritev_graph:
            context = BNode()
            piirrited_graph.add((PointOfInterest, saref.hasProperty, context))
    ...
    for saref_property in piirrited_graph.objects(PointOfInterest, saref.hasProperty):
    ...
    #         :ptoi_1 a piirrite:PointOfInterest ;
    ...
    for tuic, tuic_value in tuic_candidates.items():
        if tuic in possible_tuics:
            ...
            for property in piirrited_graph.objects(PointOfInterest, saref.hasProperty):
...
def add_PointOfInterest_to_piirrited(osm_node:URIRef,
                               osmd_graph:Graph,
                               piirrite_graph:Graph,
                               piirritev_graph:Graph,
                               piirrited_graph:Graph,
                               unfounds:dict[str, dict[str, int]]) -> dict[str, dict[str, int]]:
    ptoi_id = 'ptoi_' + str(osm_node).split('/')[-1]
    PointOfInterest = piirrited[ptoi_id]

    piirrited_graph.add((PointOfInterest, RDF.type, piirrite.PointOfInterest))

    for p, o in osmd_graph.predicate_objects(osm_node):
        if p == geo.hasGeometry:
            add_geometry_to_PointOfInterest(piirrited_graph, osmd_graph, PointOfInterest, o)
        
        elif isinstance(p, URIRef) and str(p).startswith(str(osm)) and 'wiki/Key:' in str(p):
            # if 'addr:' in str(p) or 'operator' in str(p):
            #     continue
            unfounds = add_context_to_PointOfInterest(
                osm_node, piirrite_graph, piirritev_graph, piirrited_graph, osmd_graph,
                PointOfInterest, str(p).split('wiki/Key:')[-1], str(o), unfounds
            )
...
def use_osm_data_to_fill_in_piirrited_graph(piirrite_graph:Graph,
    ...
    for count, osm_node in enumerate(osm_nodes):
        if isinstance(osm_node, URIRef):
            # TODO déterminer si on a affaire à un PointOfInterest ou juste un Point
            unfounds = add_PointOfInterest_to_piirrited(osm_node, osmd_graph, piirrite_graph,

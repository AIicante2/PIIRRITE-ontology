...
def execute_file(filepath:str) -> bool:
    ''' Executes a file.
    Args:
        filepath (str) : The absolute path of the file to execute.
    Returns:
        bool : Wether the file exists, is executable and was executed without any execution error.'''
    ...
    if subprocess.run([sys.executable, filepath], check = True).returncode != 0:
        print('3')
        return False
...
def copyfile(source:str, destination:str) -> None:
    ''' Copies a file from a given directory to another.
    If a file with the same name as the source file already exists in the destination, it is replaced.
    Args:
        source (str) : the source file absolute path.
        destination (str) : the destination file absolute path.
    '''

...
osm_file = get_current_path() + '/data/osm.ttl'
meteo_file = get_current_path() + '/data/meteo.json'
pollen_file = get_current_path() + '/data/pollen.csv'
...
def init_osm_graph() -> Graph:
    ...
    osm_graph = Graph()
    osm_graph.parse(osm_file, 'turtle')
    
    print('Données OSM récupérées.')
    return osm_graph


def get_pollen_data() -> list[dict]:
    with open(pollen_file, 'r', encoding='utf-8') as f:
        reader = csv.DictReader(f)
        pollen_data = [row for row in reader]
    return pollen_data


def get_meteo_data() -> list[dict]:
    with open(meteo_file, "r", encoding = "utf-8") as f:
        return json.load(f)


def init_GoT_graph() -> Graph:
    print('Récupération du glossaire SKOS…')
    pcv_graph = Graph()
    pcv_graph.bind('pcv', pcv)
    pcv_graph.bind('saref', saref)
    pcv_graph.bind('skos', SKOS)
    pcv_graph.parse(GoT_file, 'turtle')

    print('Glossaire SKOS récupéré.')
    return pcv_graph
...
def add_osm_property_to_FoI(
    FoI_URI:URIRef,
    osm_property:str,
    data_graph:Graph,
    pcv_graph: Graph
) -> str | None:
    osm_data = osm_property.split(':')
    key = osm_data[0]
    subkey = osm_data[1]
    value = osm_data[2] if len(osm_data) > 2 else None

    FeatureKind_URI = has_FeatureKind_URI(key, pcv_graph)
    Property_URI = has_Property_URI(key, pcv_graph)
    if FeatureKind_URI:
        data_graph.add((FoI_URI, saref.hasFeatureKind, FeatureKind_URI))

        Property_URI = has_Property_URI(subkey, pcv_graph)
        if Property_URI:
            property_value = BNode()
            data_graph.add((FoI_URI, saref.hasPropertyValue, property_value))
            data_graph.add((property_value, saref.isValueOfProperty, Property_URI))

            if value:
                Value_URI = has_Value_URI(value, pcv_graph)
                if Value_URI:
                    data_graph.add((property_value, saref.hasValue, Value_URI))
                else:
                    data_graph.add((property_value, saref.hasValue, Literal(value)))
            
            # On doit retenir les propriétés que l'élément a
            # parce que les tuic existent
            return subkey
    
    elif not FeatureKind_URI and Property_URI:
        property_value = BNode()
        data_graph.add((FoI_URI, saref.hasPropertyValue, property_value))
        data_graph.add((property_value, saref.isValueOfProperty, Property_URI))

        Value_URI = has_Value_URI(subkey, pcv_graph)
        if Value_URI:
            data_graph.add((property_value, saref.hasValue, Value_URI))
        else:
            data_graph.add((property_value, saref.hasValue, Literal(subkey)))

        return key
    else:
        FeatureKind_URI = has_FeatureKind_URI(subkey, pcv_graph)
        if FeatureKind_URI:
            data_graph.add((FoI_URI, saref.hasFeatureKind, FeatureKind_URI))


def has_FeatureKind_URI(element: str, pcv_graph: Graph) -> URIRef | None:
    uri = pcv["K_" + osm_identifier_to_local_name(element)]
    if (uri, RDF.type, saref.FeatureKind) in pcv_graph:
        return uri
    
    return None


def has_Property_URI(element:str | None, pcv_graph:Graph) -> URIRef | None:
    if not element: return None

    uri = pcv["P_" + osm_identifier_to_local_name(element)]
    if (uri, RDF.type, saref.Property) in pcv_graph:
        return uri
    uri = pcv["S_" + osm_identifier_to_local_name(element)]
    if (uri, RDF.type, saref.Property) in pcv_graph:
        return uri
...
def has_Value_URI(element:str | None, pcv_graph:Graph) -> URIRef | None:
    if not element: return None

    hash_id = hashlib.md5(f"{osm_identifier_to_local_name(element)}".encode()).hexdigest()
    uri = pcv[f"V_{hash_id}"]
    if (uri, SKOS.inScheme, pcv.Values) in pcv_graph:
        return uri
    
    return None


def osm_identifier_to_local_name(osm_identifier: str) -> str:
    s = osm_identifier.replace('-', '_')
...
def add_ElementOfInterest(
    ...
    Properties = []
    for osm_property in osm_properties:
        Property = add_osm_property_to_FoI(FoI_URI, osm_property, data_graph, pcv_graph)
        if Property: Properties.append(Property)
...
def detect_potential_ElementOfInterest(
    ...
    osm_properties = get_properties(FoI_OSM_URI, osm_graph)
    ...
    if geometry_as_wkt:
        if 'node' in str(FoI_OSM_URI) and 'POINT' in geometry_as_wkt:
            ...
            if osm_properties:
                if any(property in aoi_properties for property in osm_properties) and 'POLYGON' in geometry_as_wkt:
                    add_ElementOfInterest(
                        data_graph,
                        pcv_graph,
                ...
                elif any(property in path_properties for property in osm_properties):
                    add_ElementOfInterest(
                        data_graph,
                        pcv_graph,
...
def use_osm_data_to_fill_in_data_graph(
    ...
    for count, node in enumerate(osm_nodes):
        if isinstance(node, URIRef):
            detect_potential_ElementOfInterest(node, osm_graph, data_graph, pcv_graph, [])
    ...
    for count, osm_way in enumerate(osm_ways):
        if isinstance(osm_way, URIRef):
            paths_to_process = detect_potential_ElementOfInterest(osm_way, osm_graph, data_graph, pcv_graph, paths_to_process)
...
def circle_to_polygon_wkt(circle: str, num_points: int = 36) -> str:
    match = re.match(r"CIRCLE\(([^,]+),([^,]+),([^)]+)\)", circle)
    if match is None:
        return ''
    
    lat, lon, radius = map(float, match.groups())

    # Conversion du rayon en degrés
    # 1 degré de latitude ≈ 111320 mètres
    radius_lat = radius / 111320
    radius_lon = radius / (111320 * math.cos(math.radians(lat)))
...
def point_to_polygon_wkt(lat: str, lon: str, shape: dict[str, str | int]) -> str:
    if shape['shape'] == 'circle':
        return circle_to_polygon_wkt(f'CIRCLE({lat},{lon},{shape['radius']})')
    
    return ''


def use_pollen_data_to_fill_in_data_graph(pollen_data:list[dict], data_graph:Graph) -> None:
    for line in pollen_data:
        AoI_URI = base['aoi_' + line['zone_id']]
        data_graph.add((AoI_URI, RDF.type, piirrite.AreaOfInterest))

        # Géometrie
        geometry = BNode()
        geometry_as_wkt = circle_to_polygon_wkt(line['shape'])
        data_graph.add((geometry, geo.asWKT, Literal(geometry_as_wkt, datatype = geo['wktLiteral'])))
        data_graph.add((geometry, RDF.type, sf.Polygon))
        data_graph.add((AoI_URI, geo.hasGeometry, geometry))

        # Propriétés contextuelles
        data_graph.add((AoI_URI, saref.hasFeatureKind, pcv['K_pollen']))
        pollen_intensity = BNode()
        data_graph.add((pollen_intensity, saref.isValueOfProperty, pcv['P_pollen_intensity']))
        data_graph.add((pollen_intensity, saref.hasValue, Literal(line['intensity'], datatype = XSD.float)))
        data_graph.add((AoI_URI, saref.hasPropertyValue, pollen_intensity))
        pollen_type = BNode()
        data_graph.add((pollen_type, saref.isValueOfProperty, pcv['P_pollen_type']))
        data_graph.add((pollen_type, saref.hasValue, Literal(line['pollen_type'], datatype = XSD.string)))
        data_graph.add((AoI_URI, saref.hasPropertyValue, pollen_type))

        # Validité temporelle
        date_begin = date.fromisoformat(line["valid_from"])
        date_end   = date_begin + timedelta(days = int(line["valid_duration_days"]))

        def make_hash(*values: str) -> str:
            content = "_".join(values)
            return hashlib.sha256(content.encode()).hexdigest()[:12]
        
        hash_begin = make_hash(str(date_begin))
        hash_end = make_hash(str(date_end))
        hash_interval = make_hash(str(date_begin), str(date_end))

        instant_begin = base[f"instant_{hash_begin}"]
        instant_end = base[f"instant_{hash_end}"]
        interval = base[f"interval_{hash_interval}"]

        data_graph.add((instant_begin, RDF.type, time.Instant))
        data_graph.add((instant_begin, time.inXSDDate, Literal(date_begin, datatype = XSD.date)))

        data_graph.add((instant_end, RDF.type, time.Instant))
        data_graph.add((instant_end, time.inXSDDate, Literal(date_end, datatype = XSD.date)))

        data_graph.add((interval, RDF.type, time.Interval))
        data_graph.add((interval, time.hasBeginning, instant_begin))
        data_graph.add((interval, time.hasEnd, instant_end))
        data_graph.add((pollen_intensity, piirrite.hasTemporalValidity, interval))


def use_meteo_data_to_fill_in_data_graph(meteo_data: list[dict], data_graph: Graph) -> None:
    def make_hash(*values: str) -> str:
        content = "_".join(values)
        return hashlib.sha256(content.encode()).hexdigest()[:12]

    for line in meteo_data:
        # Création de l'URI de l'AoI à partir de la position
        lat = line['location']['latitude']
        lon = line['location']['longitude']
        timestamp = line['timestamp']
        location_hash = make_hash(str(lat), str(lon), str(timestamp))
        AoI_URI = base[f"aoi_{location_hash}"]
        data_graph.add((AoI_URI, RDF.type, piirrite.AreaOfInterest))

        # Géométrie (point converti en polygone)
        geometry = BNode()
        geometry_as_wkt = point_to_polygon_wkt(lat, lon, {'shape': 'circle', 'radius': 3000})
        data_graph.add((geometry, geo.asWKT, Literal(geometry_as_wkt, datatype=geo['wktLiteral'])))
        data_graph.add((geometry, RDF.type, sf.Polygon))
        data_graph.add((AoI_URI, geo.hasGeometry, geometry))

        # Propriétés contextuelles
        data_graph.add((AoI_URI, saref.hasFeatureKind, pcv['K_meteo']))

        temperature = BNode()
        data_graph.add((temperature, saref.isValueOfProperty, pcv['P_temperature']))
        data_graph.add((temperature, saref.hasValue, Literal(line['temperature_c'], datatype=XSD.float)))
        data_graph.add((AoI_URI, saref.hasPropertyValue, temperature))

        humidity = BNode()
        data_graph.add((humidity, saref.isValueOfProperty, pcv['P_humidity']))
        data_graph.add((humidity, saref.hasValue, Literal(line['humidity_percent'], datatype=XSD.float)))
        data_graph.add((AoI_URI, saref.hasPropertyValue, humidity))

        wind_speed = BNode()
        data_graph.add((wind_speed, saref.isValueOfProperty, pcv['P_wind_speed']))
        data_graph.add((wind_speed, saref.hasValue, Literal(line['wind_speed_kmh'], datatype=XSD.float)))
        data_graph.add((AoI_URI, saref.hasPropertyValue, wind_speed))

        wind_direction = BNode()
        data_graph.add((wind_direction, saref.isValueOfProperty, pcv['P_wind_direction']))
        data_graph.add((wind_direction, saref.hasValue, Literal(line['wind_direction'], datatype=XSD.string)))
        data_graph.add((AoI_URI, saref.hasPropertyValue, wind_direction))

        precipitation = BNode()
        data_graph.add((precipitation, saref.isValueOfProperty, pcv['P_precipitation']))
        data_graph.add((precipitation, saref.hasValue, Literal(line['precipitation_mm'], datatype=XSD.float)))
        data_graph.add((AoI_URI, saref.hasPropertyValue, precipitation))

        cloud_cover = BNode()
        data_graph.add((cloud_cover, saref.isValueOfProperty, pcv['P_cloud_cover']))
        data_graph.add((cloud_cover, saref.hasValue, Literal(line['cloud_cover_percent'], datatype=XSD.float)))
        data_graph.add((AoI_URI, saref.hasPropertyValue, cloud_cover))

        pressure = BNode()
        data_graph.add((pressure, saref.isValueOfProperty, pcv['P_pressure']))
        data_graph.add((pressure, saref.hasValue, Literal(line['pressure_hpa'], datatype=XSD.float)))
        data_graph.add((AoI_URI, saref.hasPropertyValue, pressure))

        weather_condition = BNode()
        data_graph.add((weather_condition, saref.isValueOfProperty, pcv['P_weather_condition']))
        data_graph.add((weather_condition, saref.hasValue, Literal(line['weather_condition'], datatype=XSD.string)))
        data_graph.add((AoI_URI, saref.hasPropertyValue, weather_condition))

        # Validité temporelle (le timestamp représente l'instant de mesure)
        time_begin = datetime.fromisoformat(line["timestamp"])
        time_end   = time_begin + timedelta(hours = 1)

        def make_hash(*values: str) -> str:
            content = "_".join(values)
            return hashlib.sha256(content.encode()).hexdigest()[:12]
        
        hash_begin = make_hash(str(time_begin))
        hash_end = make_hash(str(time_end))
        hash_interval = make_hash(str(time_end), str(time_end))
        ...
        data_graph.add((instant_begin, RDF.type, time.Instant))
        data_graph.add((instant_begin, time.inXSDDateTimeStamp, Literal(time_begin, datatype = XSD.dateTime)))

        data_graph.add((instant_end, RDF.type, time.Instant))
        data_graph.add((instant_end, time.inXSDDateTimeStamp, Literal(time_end, datatype = XSD.dateTime)))

        data_graph.add((interval, RDF.type, time.Interval))
        data_graph.add((interval, time.hasBeginning, instant_begin))
        data_graph.add((interval, time.hasEnd, instant_end))

        # On associe la validité temporelle à chaque propriété
        for prop_bnode in [
            temperature, humidity, wind_speed, wind_direction,
            precipitation, cloud_cover, pressure, weather_condition
        ]:
            data_graph.add((prop_bnode, piirrite.hasTemporalValidity, interval))
...
def main():
    ...
    pcv_graph = init_GoT_graph()
    data_graph = init_data_graph()

    use_osm_data_to_fill_in_data_graph(osm_graph, pcv_graph, data_graph)
    use_pollen_data_to_fill_in_data_graph(pollen_data, data_graph)
    use_meteo_data_to_fill_in_data_graph(meteo_data, data_graph)
